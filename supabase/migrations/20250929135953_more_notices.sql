    ...
    if TG_TABLE_NAME = 'profiles' then
        ...
        if v_role_id is not null then
            ...
            for user_id in select public.profiles.user_id from public.profiles where public.profiles.role_id = v_role_id loop
                ...
                -- Diagnostic: print final permissions value for test user
                if user_id = '00000000-0000-0000-0000-000000000001' then
                    raise notice '[role_permissions] Final permissions for test user: %', (select raw_app_meta_data->'permissions' from auth.users where id = user_id);
                end if;
            end loop;
        end if;
        ...
        for user_id in select p.user_id from public.profiles p left join public.role_permissions rp on rp.role_id = p.role_id where p.role_id is not null and rp.role_id is null loop
            ...
            -- Diagnostic: print final permissions value for test user after sweep
            if user_id = '00000000-0000-0000-0000-000000000001' then
                raise notice '[role_permissions sweep] Final permissions for test user: %', (select raw_app_meta_data->'permissions' from auth.users where id = user_id);
            end if;
        end loop;
