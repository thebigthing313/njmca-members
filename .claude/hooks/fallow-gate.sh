...
# Generated by fallow setup-hooks, then patched locally. Search LOCAL PATCH for
# the divergences; reinstalling this hook from fallow will discard them.
# Installer version: 3.17.0
...
is_git_write_command() {
  ...
  while IFS= read -r segment; do
    ...
    set -- $segment
    while [ "$#" -gt 0 ]; do
      # LOCAL PATCH: git.exe is how the command arrives on Windows.
      case "$1" in
        git | git.exe) ;;
        *)
          shift
...
# LOCAL PATCH: resolve the repo's pinned fallow before any global install, so the
# agent gate and .githooks/pre-commit always audit with the same binary.
GATE_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
if [ -x "$GATE_ROOT/node_modules/.bin/fallow" ]; then
  RUNNER=("$GATE_ROOT/node_modules/.bin/fallow")
  BIN_DESC="$GATE_ROOT/node_modules/.bin/fallow"
...
# LOCAL PATCH: --version prints a second `verified:` line; without head -n1 the
# newline survives into VERSION and the sort -V floor check passes only by luck.
